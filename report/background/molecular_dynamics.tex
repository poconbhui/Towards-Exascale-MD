...
\begin{figure}
    \begin{center}
        \begin{tikzpicture}[node distance = 4cm, auto]
            % Place nodes
            \node [block] (init) {Initialise simulation};
            \node [block, right of=init] (equilibrate) {Equilibrate system};
            \node [block, right of=equilibrate] (perform) {
                Simulate and measure system
            };
            % Place paths
            \path [line] (init) -- (equilibrate);
            \path [line] (equilibrate) -- (perform);
        \end{tikzpicture}
    \end{center}
...
performing a summation over $N-1$ positions $\vec{x}_j$.
...
truncation and boundary conditions are outside the scope of
this dissertation.
...
model a particular system of particles, but instead over-approximates
